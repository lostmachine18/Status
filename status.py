...
if choose_analysis == "Repeated measures mixed ANOVA":
    ...
    x_var = st.sidebar.selectbox("Choose numetic depandent variable:", (numeric_vars))
...
if choose_analysis == "Correlation":
    ...
    expand = st.sidebar.beta_expander("More options")
    method_selected = expand.radio("Select correlation type", ("pearson", "spearman", "kendall", "bicor", "percbend", "shepherd"))
    ...
    if plot_r:
        g.ax_joint.text(x_pos, y_pos, f"r = {corr_result.iloc[0].loc['r'].round(3)}, p = {corr_result.iloc[0].loc['p-val'].round(3)}", fontstyle='italic')
